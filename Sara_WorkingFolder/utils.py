...
def get_save_wiki_docs(keywords, save_folder = 'data/wiki_data2/'):
    ...
    for i, kw in enumerate(keywords):
        ...
        wiki_list = wiki.search(kw)
        for page in wiki_list[:3]:
            try:
                content = wiki.page(page).content.encode('ascii', 'ignore')
            except wiki.exceptions.DisambiguationError as e:
                print 'DisambiguationError', page
            except:
                print 'Error', page
            if not content:
                continue
            with open(os.path.join(save_folder, '_'.join(page.split()) + '.txt'), 'w') as f:
                        f.write(content)
